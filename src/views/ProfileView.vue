          ...
          <form @submit.prevent="saveProfile" class="space-y-6">
            ...
            <div>
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4 items-end">
                <div>
                  <label class="block text-xs font-medium text-slate-700 mb-1">Nova Senha</label>
                  <input 
                    v-model="newPassword" 
                    type="password" 
                ...
                <div>
                  <button 
                    ...
                    class="w-full py-2.5 px-4 bg-slate-100 hover:bg-slate-200 text-slate-800 font-semibold rounded-xl text-xs transition-all disabled:opacity-50 flex items-center justify-center gap-2 border border-slate-200"
                  >
                    <Loader2 v-if="savingPassword" class="w-3.5 h-3.5 animate-spin" />
                    <span>{{ savingPassword ? 'Atualizando...' : 'Definir / Alterar Senha' }}</span>
                  </button>
                </div>
              </div>
            </div>
...
const updatePassword = async () => {
  if (!newPassword.value || newPassword.value.length < 4) {
    toast.error('Senha muito curta', { description: 'Informe uma senha com no mínimo 4 caracteres.' })
    return
  }
  savingPassword.value = true
  try {
    await authStore.setPassword(newPassword.value)
    toast.success('Senha Atualizada!', { description: 'Sua nova senha foi definida com sucesso.' })
    newPassword.value = ''
  } catch (err) {
    toast.error('Erro ao Atualizar Senha', { description: authStore.error || 'Não foi possível definir a senha.' })
  } finally {
